...
int main() {
  ...
  sf::RenderWindow window(sf::VideoMode::getDesktopMode(), "Memo",
                          sf::Style::Fullscreen);
  ...
  while (window.isOpen()) {
    ...
    // ImGui window
    if (is_open) {
      ImGui::Begin("Deck - Current card", &is_open,
      ...
      if (cards->getNbCards() != 0) {
        ImGui::SetCursorPosX(
            (ImGui::GetWindowSize().x - ImGui::CalcTextSize("Details").x) *
            0.5f);
        ImGui::Text("Details");
        ImGui::Separator();
        ImGui::NewLine();
        ...
        ImGui::Text("Last Seen: %ld days ago",
                    (time(nullptr) - c->getLastSeen()) / DAY_IN_SECONDS);
      ...
      ImGui::SetCursorPosX(
          (ImGui::GetWindowSize().x - ImGui::CalcTextSize("Current Card").x) *
          0.5f);
      ...
      if (ImGui::Button(question.c_str(),
                        ImVec2(ImGui::GetWindowSize().x * 0.9f, 100.0f))) {
        if (cards->getNbCards() != 0) {
      ...
      ImGui::NewLine();
      static unsigned int nextQuestion = 1;
      ...
      ImGui::NewLine();
      ImGui::SetCursorPosX(
          (ImGui::GetWindowSize().x - ImGui::CalcTextSize("Card Actions").x) *
          0.5f);
      ...
      if (ImGui::Button("Next Card",
                        ImVec2(ImGui::GetWindowSize().x * 0.9f, 40.0f))) {
        if ((unsigned int)nextQuestion != cards->getNbCards() &&
            cards->getNbCards() != 0) {
      ...
      if (ImGui::Button("Delete Card",
        ...
        if (cards->getNbCards() > 1) {
          ...
          if ((unsigned int)currQuestion == cards->getNbCards() &&
              currQuestion != 0)
            currQuestion--;
    ...
    ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.9f);
    ImGui::SetCursorPosX(
        (ImGui::GetWindowSize().x - ImGui::CalcTextSize("Details").x) * 0.5f);
    ImGui::Text("Details");
    ImGui::Separator();
    ImGui::NewLine();
    ...
    ImGui::NewLine();
    ImGui::SetCursorPosX(
        (ImGui::GetWindowSize().x - ImGui::CalcTextSize("Current Deck").x) *
        0.5f);
    ImGui::Text("Current Deck");
    ImGui::Separator();
    ImGui::NewLine();
    static std::string deck = cards->getDeckName();
    ImGui::Button(deck.c_str(),
                  ImVec2(ImGui::GetWindowSize().x * 0.9f, 100.0f));
    ImGui::NewLine();
    static int unsigned nextDeck = 1;
    ImGui::PushStyleColor(ImGuiCol_Button, BLACK);
    if (ImGui::Button("Next Deck",
                      ImVec2(ImGui::GetWindowSize().x * 0.9f, 40.0f))) {
    ...
    if (ImGui::BeginCombo("##decks", preview_d)) {
      for (unsigned int j = 0; j < decks.size(); ++j) {
    ...
    ImGui::NewLine();
    ImGui::SetCursorPosX(
        (ImGui::GetWindowSize().x - ImGui::CalcTextSize("Deck Actions").x) *
        0.5f);
    ImGui::Text("Deck Actions");
    ImGui::Separator();
    ImGui::NewLine();
    ImGui::PushStyleColor(ImGuiCol_Button, BLUE);
    if (ImGui::Button("Create New Deck",
                      ImVec2(ImGui::GetWindowSize().x * 0.9f, 40.0f)))
